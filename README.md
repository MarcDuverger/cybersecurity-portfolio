👋 Marc Duverger — Portfolio Cybersécurité
> Reconversion pro dans la cybersécurité orientation **Test d'intrusion / Red Team**. Je détecte les attaques *et* je sais les mener.
---
🧭
Reconverti de l'audiovisuel vers la cybersécurité, je suis certifié à la Guardia Cybersecurity School (Bloc 3 — AIS). Je prépare actuellement la certification eJPT (eLearnSecurity Junior Penetration Tester).
Mon approche : comprendre les deux côtés de la sécurité. Côté défense, je construis et supervise des environnements de détection (SIEM, IDS, Active Directory). Côté offensive, je m'entraîne sur des laboratoires vulnérables en documentant chaque étape comme un vrai livrable de mission.
Domaines : SOC · SIEM (Wazuh) · IDS/IPS (Suricata) · Active Directory · Test d'intrusion · Analyse de vulnérabilités · Réponse à incident
---
📂 Projets
🔵 Blue Team / SOC
Projet	Description	Compétences
Audit Blue Team — BFC	Audit défensif complet : durcissement AD, SIEM, IDS, réponse à incident	Wazuh, Suricata, OPNsense, AD, MITRE ATT&CK, ANSSI
🔴 Red Team / Pentest
Machine	Plateforme	Compétences travaillées
DC-1	VulnHub	Énumération web, exploitation CMS (Drupalgeddon), élévation de privilèges Linux
Mr Robot	VulnHub	Énumération WordPress, brute-force, cassage de hash, SUID
---
🎯 Certifications
✅ Guardia Cybersecurity School — AIS (Bloc 3), 2026
🎯 eJPT — en préparation
---
🛠️ Compétences techniques
Défense : Wazuh (SIEM) · Suricata (IDS/IPS) · OPNsense · Active Directory · GPO / durcissement ANSSI · MFA · fail2ban · Lynis · Purple Knight
Offensive : Nmap · exploitation web · Metasploit / searchsploit · reverse shells · élévation de privilèges Linux · WPScan · John the Ripper · Hydra · Gophish
Méthodologie : MITRE ATT&CK · NIST CSF · Zero Trust · rédaction de rapports d'audit et de pentest
---
📫 Contact
LinkedIn : linkedin.com/in/marc-duverger-cyber
Email : marcduverger.ynwa@gmail.com
- **CV :** [Télécharger (PDF)](./CV_Marc_Duverger_Public.pdf)
---
Tous les tests présentés dans ce portfolio ont été réalisés dans des environnements de laboratoire isolés, à des fins pédagogiques et légales.
